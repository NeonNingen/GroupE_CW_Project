...
/**
 *
 * @author Aisana (w1775375) code for DlgHist list is made by Rafael ()
 */
public class DialogueCont implements ActionListener {
    ...
    /**
     * by Aisana (w1775375) Method triggered when dlg list row is clicked = dlg
     * is chosen Method will display option pane with information on the
     * dialogue and option to choose yes/no clicking yes will proceed to the
     * setup page, no will return to dlg list
     *
    ...
    /**
     * by Aisana (w1775375) Method is used to add or remove mouse listener when
     * needed using booleans true - means to add mouse listener, false - means
     * to remove it
     */
    ...
    /**
     * by Aisana (w1775375) When initialising the controller its constructor
     * will create instance of dialogue Model and dialogue list view classes in
     * order to generate arrayList of Dialogues and generate JTable in the
     * dialogue view JFrame
     *
     * @param query - string that will contain a query needed to access the
     * needed table from database
     */
    ...
    /**
     * by Aisana (w1775375) Method to write down list of dialogues used for
     * checking at the back end
     */
    ...
    /**
     * by Aisana (w1775375) Method will connect to database and retrieve
     * existing values and store them into comboboxes that will be used to
    ...
    /**
     * by Aisana (w1775375) tracks actions with comboboxes and reset button
     *
    ...
    /**
     * by Aisana (w1775375) Method will check comboboxes selected options for
     * setQuery() method and trigger filterDlgList() to filter data and display
     * filtered table
     *
     * @param str - used to check if user wants to filter or reset comboboxes,
     * write string to be "reset" to reset comboboxes
     */
    ...
    /**
     * by Aisana (w1775375) Method will reset all comboboxes to default row at
     * the page(useful for resting all combboboxes at once)
     */
    ...
    /**
     * by Aisana (w1775375) return false if default option is set (means no
     * meaningful option was chosen) it will return true if anything apart from
     * default option in row 0 has been selected
     *
     * @param box - use string inside of a combobox to check if combobox is set
     * at default or not
     * @return
     */
    ...
    /**
     * by Aisana (w1775375) Method used to set up query for a filter based on 3
     * comboboxes selected values
     *
    ...
    /**
     * by Aisana (w1775375) Method will store a draft list of dialogues based on
     * query given and display table with filtered dialogue list
     *
     * @param dlgView - user to reference the same page view that was previously
     * created
     * @param dlgModel - user to reference the same dialogue model class that
     * was originally created
     * @param query - a string with specified query, needed to sort database
     * table and return requiered dialogues
     */
    ...
    /**
     * Suggested by Aisana for Rafael(as addition to menubar) Use this method to
     * set up dialogue history list and display it into table note that menuBar
     * controller will refer to this method to initialise dialogue hist list
     * view Aisana created (as author of menubar), for any issues address her
     *
     * @param histList
     * @param Model
     */
